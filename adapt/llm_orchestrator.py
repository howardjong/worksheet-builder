...
class AdaptationLogEntry(BaseModel):
    ...
    specific_skill: str
    template_type: str
    outcome: str  # gemini_first_try | gemini_retry | gpt_takeover | deterministic_fallback | llm_failure
    gemini_attempts: int
...
def orchestrate_llm_adaptation(
    ...
    if gemini_available:
        for attempt in range(MAX_GEMINI_ATTEMPTS):
            ...
            if attempt == 0 or not verdicts:
                logger.info("  LLM orchestrator: Gemini attempt %d/%d", attempt + 1, MAX_GEMINI_ATTEMPTS)
                plan, worksheets = _gemini_plan(skill, profile, theme_id, rules, rag_curriculum_references)
            else:
                logger.info("  LLM orchestrator: Gemini retry %d/%d with judge feedback", attempt + 1, MAX_GEMINI_ATTEMPTS)
                plan, worksheets = _gemini_plan_with_feedback(
                    skill, profile, theme_id, rules,
                    verdicts[-1],
                    rag_curriculum_references,
                )

            if plan is None or worksheets is None:
                logger.warning("  LLM orchestrator: Gemini attempt %d failed to produce a plan", attempt + 1)
                continue
            ...
            if gpt_available:
                ...
                if verdict is not None:
                    ...
                    if verdict.approved:
                        ...
                        logger.info("  LLM orchestrator: %s (score=%.2f)", outcome, verdict.overall_score)
                        _log_performance(
                            _build_log_entry(skill, outcome, gemini_attempts, verdicts, verdict.overall_score, "gemini-3-flash-preview"),
                            artifacts_dir,
                        )
                        return worksheets
                    else:
                        logger.warning("  LLM orchestrator: Gemini attempt %d rejected (score=%.2f)", attempt + 1, verdict.overall_score)
                else:
                    # Judge call itself failed — accept Gemini's plan
                    logger.warning("  LLM orchestrator: judge unavailable, accepting Gemini plan")
                    _log_performance(
                        _build_log_entry(skill, "gemini_first_try", gemini_attempts, verdicts, None, "gemini-3-flash-preview"),
                        artifacts_dir,
                    )
                    return worksheets
            else:
    ...
    if gpt_available:
        ...
        if worksheets:
            _log_performance(
                _build_log_entry(skill, "gpt_takeover", gemini_attempts, verdicts, None, "gpt-5.4"),
                artifacts_dir,
            )
            return worksheets
        else:
    ...
    _log_performance(
        _build_log_entry(skill, "llm_failure", gemini_attempts, verdicts, None, "none", "Both LLM paths exhausted"),
        artifacts_dir,
    )
    return None
...
def _gemini_plan_with_feedback(
    ...
    logger.info("  Gemini retry planned %d worksheets — %s", len(plan.worksheets), plan.concept_focus)
...
def _gpt_plan(
    ...
    logger.info("  GPT takeover planned %d worksheets — %s", len(plan.worksheets), plan.concept_focus)
