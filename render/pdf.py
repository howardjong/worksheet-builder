...
def render_cover_page(
    ...
    """Render a cover page for a lesson package.
    ...
    # --- Cover image (top ~45%) ---
    image_bottom = PAGE_HEIGHT * 0.52  # image occupies top 48% of page
    if cover_image_path and Path(cover_image_path).exists():
        img_x = MARGIN
        img_y = image_bottom
        img_w = CONTENT_WIDTH
        img_h = CONTENT_TOP - image_bottom
        try:
            c.drawImage(
                cover_image_path,
                img_x,
            ...
            _draw_cover_fallback(c, theme, img_x, img_y, img_w, img_h)
    else:
        img_x = MARGIN
        img_y = image_bottom
        img_w = CONTENT_WIDTH
    ...
    # --- Title ---
    title = _derive_cover_title(skill_model, worksheets)
    c.setFont(theme.fonts.heading, 36)
    c.setFillColor(HexColor(colors.directions))
    _apply_adhd_spacing(c, "heading")
    # Center the title
    title_width = c.stringWidth(title, theme.fonts.heading, 36)
    if title_width > CONTENT_WIDTH:
        c.setFont(theme.fonts.heading, 28)
        title_width = c.stringWidth(title, theme.fonts.heading, 28)
    ...
    c.setFont(theme.fonts.heading, 16)
    c.setFillColor(HexColor(colors.text))
    c.drawString(MARGIN, y, "What's Inside:")
    y -= 24

    c.setFont(theme.fonts.primary, 14)
    _apply_adhd_spacing(c, "body")
    worksheet_labels = {
        "Word Discovery": "Match words to pictures!",
        "Word Builder": "Build new words!",
        "Story Time": "Read a cool story!",
    }
    for i, ws in enumerate(worksheets, start=1):
        ws_title = ws.worksheet_title or f"Worksheet {i}"
        desc = worksheet_labels.get(ws_title, "Practice time!")
        line = f"{i}. {ws_title} -- {desc}"
        c.setFillColor(HexColor(colors.text))
        c.drawString(MARGIN + 10, y, line)
        y -= 22

    # --- Parent/teacher info strip ---
    y = MARGIN + 40
    c.setFont(theme.fonts.primary, 10)
    c.setFillColor(HexColor(colors.chunk_border))

    info_parts = [
        f"Grade {skill_model.grade_level}",
        skill_model.domain.replace("_", " ").title(),
        skill_model.specific_skill.replace("_", " "),
    ]
    if profile_name:
        info_parts.insert(0, f"For {profile_name}")
    c.drawString(MARGIN, y, " | ".join(info_parts))
    y -= 14

    if skill_model.learning_objectives:
        obj_text = "Objectives: " + "; ".join(skill_model.learning_objectives[:3])
        if len(obj_text) > 90:
            obj_text = obj_text[:87] + "..."
        c.drawString(MARGIN, y, obj_text)
        y -= 14

    c.drawString(MARGIN, y, "Print all pages at 100% on letter paper")
...
def _draw_cover_fallback(
    ...
    """Draw a theme-colored placeholder when no cover image is available."""
    # Soft rounded rectangle with theme accent color
    bg_color = HexColor(theme.colors.reading_bg)
    border_color = HexColor(theme.colors.reading_border)
    c.setFillColor(bg_color)
    c.setStrokeColor(border_color)
    c.setLineWidth(2)
    c.roundRect(x, y, w, h, radius=12, fill=True, stroke=True)

    # Center text
    c.setFont(theme.fonts.heading, 24)
    c.setFillColor(HexColor(theme.colors.directions))
    label = "Let's Learn!"
    lw = c.stringWidth(label, theme.fonts.heading, 24)
    c.drawString(x + (w - lw) / 2, y + h / 2, label)


def _derive_cover_title(
    skill_model: object,
    worksheets: list[AdaptedActivityModel],
) -> str:
    """Derive a fun cover title from the lesson content."""
    from skill.schema import LiteracySkillModel

    assert isinstance(skill_model, LiteracySkillModel)

    # Try to use decodable story title from the story worksheet
    ...
    skill_name = skill_model.specific_skill.replace("_", " ").title()
    if skill_model.target_words:
        sample = ", ".join(skill_model.target_words[:3])
        return f"Word Adventure: {sample}!"
    return f"{skill_name} Adventure!"
