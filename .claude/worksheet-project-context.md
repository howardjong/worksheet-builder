...
- **First task next session**: run a controlled alternative-voice or alternative-provider canary on the remaining pacing-only failures, starting with `lesson_014_passage_sentence_03`, `lesson_014_passage_full`, `lesson_095_passage_sentence_02`, and `lesson_095_passage_full`.
- **Second task after that**: decide whether the pilot gate should remain strict at the current passage-family thresholds or whether a manual-review-plus-zero-blocker pilot is acceptable for a very small child test; make that decision explicitly before scaling any further.
- **Third task after that**: if Dorothy remains the chosen pilot voice, consider only light-touch final cleanup inside our control, such as slightly calmer Lesson 1 instruction pacing and a narrower review-band interpretation for very slow-but-helpful clips. Avoid more major text-shaping changes unless a canary proves they help.
- **Fourth task after that**: implement Stage 2 representative-pilot work: lesson-level aggregate documents, `audio_companion_clips`/`audio_companion_lessons` collection split, and `--granularity lessons|both`.
