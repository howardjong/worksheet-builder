...
**Next: S5 — live falsification experiment (owner env, NOT offline).** Pre-warm extraction cache, then `--runs 2` with the contract ON vs the Session 48 baseline (`20260612_220801`/`20260612_222046`) to confirm coverage-driven approve/reject flips disappear + blind engagement/repair-filler review. Command in the plan (Task S5). Then S6 (full SlotPack) only if S5 is falsified. Hold parent Tasks 13–15.
