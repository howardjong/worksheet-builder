...
- **First task next session**: design and implement a concrete fallback decision policy for UFLI audio generation. Goal: keep ElevenLabs as the default path, identify narrow pacing-risk clips cheaply, and use Gemini only as a targeted fallback for eligible `passage_sentence` / `passage_full` failures. Do not assume we can afford full LLM judging on every clip.
- **Second task after that**: define the deterministic pre-judge heuristics and thresholds. Candidate signals: clip family, actual duration, actual WPM against existing family bands, known-risk segment IDs or lesson families, and text-shaping risk markers. Separate three buckets explicitly: auto-accept, Gemini-fallback-eligible, and still-needs-LLM/manual review.
- **Third task after that**: if the fallback policy looks safe, implement an opt-in fallback path rather than an automatic provider swap. Best shape: ElevenLabs first, then targeted Gemini retry for eligible pacing-only failures, then re-judge only the fallbacked or suspicious clips.
- **Fourth task after that**: only after the fallback policy is clear, resume any Gemini prompt/rate exploration needed to fill policy gaps. Avoid broad new canaries unless they answer a concrete policy question.
