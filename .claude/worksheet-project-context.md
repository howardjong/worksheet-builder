...
- **Current code state**: `rag/backfill.py` and `rag/eval.py` are implemented; curriculum-aware adaptation is now complete in `adapt/engine.py`; multimodal corpus audit is implemented in `corpus/ufli/audit.py` + `corpus/ufli/audit_schema.py`; `corpus/ufli/audio_judge.py` is implemented and live-verified against `gemini-3-flash-preview`; the first full Dorothy audio judge run is recorded at `data/ufli/companion/evals/20260316_121249`.
- **First task next session**: fix the Stage 1 audio bundle extraction/prompting issues flagged by the live judge before any child-facing MVP sessions: filter teacher-facing labels from word targets (`activity`, `or`, `concept`, `home`, `work`), make lesson/review prompts decoding-first, slow the phoneme-model delivery for Dorothy, substantially slow `passage_sentence` / `passage_full` delivery, and correct the Lesson 95 `oy` anchor.
- **Second task after that**: rerun `build-audio`, live `generate-audio`, and `judge-audio` for `dorothy`; only then set a real `neutral_na_pilot` voice id in `data/ufli/companion/voice_profiles.yaml` and compare both voices.
- **Third task after that**: implement Stage 2 representative-pilot work: lesson-level aggregate documents, `audio_companion_clips`/`audio_companion_lessons` collection split, and `--granularity lessons|both`.
