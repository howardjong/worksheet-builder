...
**Last Updated:** 2026-03-16 (post-Google Chirp 3 and Gemini-TTS canaries)
...
- Practical conclusion from both Google canaries: keep ElevenLabs/Dorothy as the main production path for now; treat Google as diagnostics-only. Chirp 3 / Leda still looks mildly promising for some `passage_full` clips, while Gemini `gemini-2.5-pro-tts` with the tested child-directed prompt and slow rate looks too slow for child-directed passage pacing unless we materially relax the prompt and/or raise the speaking rate.
...
- **First task next session**: if Google exploration continues, run one narrower recalibration canary instead of another broad provider swap. Best candidate: keep `gemini-2.5-pro-tts` + `Leda`, shorten or soften the style prompt, remove or sharply reduce inline pause tags, and raise speaking rate closer to `0.95-1.0` on the same four focused clips.
