...
**Last Updated:** 2026-03-24 (LLM-assisted adaptation + pedagogical judge implemented)

### 2026-03-24 LLM-Assisted Adaptation + GPT 5.4 Pedagogical Judge

**Status:** Implemented and live-tested. Two-LLM architecture working end-to-end: Gemini plans worksheet structure, GPT 5.4 judges pedagogical quality. 418 tests pass, lint clean, typecheck clean.

**Problem:** The adaptation engine used rigid deterministic templates (Discovery → Builder → Story) that ignored the pedagogical intent of the source UFLI lesson. For Lesson 75 (-le pattern), the output started with generic Elkonin sound boxes (which break the -le syllable apart, contradicting the lesson) and word-picture matching instead of the lesson's core word chain activities. The deterministic code couldn't reason about what the lesson was trying to teach.

**Two-phase fix:**

#### Phase 1: Deterministic improvements (immediate fixes)
- **Reordered UFLI worksheets** (`adapt/engine.py`): For `ufli_word_work` templates with word chains, Builder (chains) now leads as "Word Work", Discovery moves to "Word Practice". Only triggers when Builder actually has chain content.
- **Skipped Elkonin warmup for -le** (`adapt/engine.py:_build_warmup_chunk`): Consonant-le syllable patterns bypass sound-box segmentation since it contradicts the lesson's teaching goal.
- **Increased word coverage** (`adapt/engine.py`): Write/trace/fill-blank now use up to 5 words (ADHD-safe max for grade 1), circle uses all 6 as options. Previously capped at 4, dropping 2 sample words entirely.

#### Phase 2: LLM-assisted adaptation (new architecture)

| Component | File | Role |
|-----------|------|------|
| **Gemini Planner** | `adapt/llm_adapt.py` | Understands source content and plans worksheet structure — activity types, ordering, word distribution, worked examples |
| **GPT 5.4 Judge** | `adapt/llm_judge.py` | Evaluates adapted worksheets across 4 dimensions: concept alignment, content coverage, lesson flow, ADHD compliance |
| **Engine integration** | `adapt/engine.py` | `adapt_lesson()` tries LLM path first, falls back to deterministic engine if Gemini unavailable or fails |
| **Pipeline integration** | `transform.py` | Runs judge after adaptation, logs verdict to `artifacts/judge_verdict.json` |

**Architecture:**
```
Source Image → Gemini Vision (extract) → Gemini Flash (plan activities)
                                              ↓
                                    GPT 5.4 Judge (evaluate)
                                        ↓ approved?     ↓ rejected?
                                      Render PDF     Deterministic fallback
```

**LLM Planner (`adapt/llm_adapt.py`):**
- Takes skill model + learner profile + ADHD rules + RAG curriculum references
- Sends rich prompt to `gemini-3-flash-preview` with full source content and ADHD constraints
- Gemini outputs a `LessonPlan` (simplified intermediate schema: worksheets → activities → words/instructions)
- Deterministic translation code converts `LessonPlan` → `AdaptedActivityModel` objects (IDs, scaffolding, decoration zones, Pydantic validation)
- Returns `None` on any failure so caller falls back to deterministic engine

**GPT 5.4 Judge (`adapt/llm_judge.py`):**
- Receives source skill model + adapted worksheets
- Evaluates 4 scored dimensions (0.0–1.0): concept_alignment, content_coverage, lesson_flow, adhd_compliance
- Approves if overall_score ≥ 0.7 and no criterion below 0.5
- Returns `JudgeVerdict` with detailed feedback items and rationale
- Currently advisory (logged to artifacts but does not block rendering)

**Live test (UFLI Lesson 75 -le, home practice):**
- Gemini planned 3 worksheets, translation produced 2 (one had no valid items)
- GPT 5.4 REJECTED (score=0.28): correctly identified missing word chains, dropped target words, weak ADHD supports — the exact same problems the user originally found with the deterministic engine
- Deterministic fallback with Phase 1 fixes produced APPROVED output (all validations pass)

**Config:**
- LLM adaptation is **opt-in**: requires `WORKSHEET_LLM_ADAPT=1` env var. Without it, the deterministic engine is used (safe for tests and CI).
- `pyproject.toml`: Added `[tool.ruff.lint.per-file-ignores]` for `adapt/llm_adapt.py` and `adapt/llm_judge.py` (E501 exemption for prompt strings)

**Test updates:** 6 test files updated for new UFLI worksheet titles ("Word Work" / "Word Practice" when chains present): `tests/test_adapt.py`, `tests/test_rag_adapt.py`, `tests/test_render.py`

**What's next:**
1. **Retry on rejection** — Feed judge feedback back to Gemini for a refined second attempt before falling back to deterministic
2. **Prompt tuning** — Gemini's first attempt dropped content; the prompt may need stronger emphasis on complete content preservation
3. **Fall back on rejection** — When judge rejects, automatically switch to deterministic engine (currently advisory only)
4. **Decodable passage adaptation** — Apply same LLM approach to `ufli_decodable_story` templates for fluency worksheets
