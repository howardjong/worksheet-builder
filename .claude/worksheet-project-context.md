...
**What happened:**
- Owner UAT of lesson 101 (`-ly`, first single/non-comparative suffix tested since lesson-100's `-er/-est`) — no override. Pre-render abort on TWO criteria, not just the known connected-text blocker: `obj_manipulation` (0.26, `wrong_cognitive_task` + `generic_activity_not_exercising_objective`) was new signal, surfaced per the pre-briefed instruction rather than silently overridden.
- Systematic-debugging root-caused via live code repro (not log inspection): **D48** — `adapt/llm_adapt.py`'s `_items_for_activity` prefers model-authored `.items` for `word_chain`, but the coverage evaluator's chain-evidence stamp (`metadata={"display": "chain_step"}`) is a deliberate anti-gaming discriminator only the deterministic parser can set (`validate/objective_coverage.py:341-347` design comment) — so an LLM-authored chain could never count as evidence, on ANY suffix or letter-chain lesson, regardless of content quality. **D49** — even the deterministic engine's correctly-rendered fallback failed the judge because the manipulation cell's `sufficiency_rule` text describes a multi-hop chain (`"≥1 coherent build/change chain"`) that single-suffix lessons structurally can't form (UFLI's `-ly` chains are single-hop pairs only, unlike `-er/-est`'s 2-hop comparative chains).
- Spec v1 (`docs/superpowers/specs/2026-07-16-lesson101-uat-fixes-design.md`) proposed relaxing the deterministic coverage evaluator for D49 — **caught by the owner asking to double-check before proceeding**, then disproven by an offline evaluator run: the deterministic layer already passes single-hop packages; the real failing layer is the judge's handed facts. v2 retargeted correctly. Worth remembering: verify a fix design against running code before committing to a plan, even (especially) after a thorough spec-writing pass.
- Plan (`docs/superpowers/plans/2026-07-16-lesson101-uat-fixes.md`), 4 tasks, first cycle with **complexity-matched model assignment** (owner-requested) instead of uniform Sonnet 5: Tasks 1-2 (mechanical, code given in the plan) → Sonnet 5; Task 3 (cross-module, byte-identical-regression-bar) → Opus 4.8; Task 4 (live-run judgment against noisy baselines) → Opus 4.8; final whole-branch review → Fable 5.
- Subagent-driven execution, all 4 task reviews clean on first pass (0 Critical/Important across the board; reviewers independently re-reproduced RED evidence by reverting fixes in the working tree, not just reading reports). Task 1: `word_chain` added to `_MECHANICAL_FORMATS` + suffix-parser parity in `adapt/llm_adapt.py` (`45f9529`). Task 2: planner prompt + retry feedback teach the arrow-`words` format (`e2e16b6`). Task 3: `_make_manipulation_cell(skill, ctx)` gains chain-shape detection, single-hop suffix lessons get add-the-ending wording, one judge-gloss guard line (`eb2d87e`).
- Live acceptance (Task 4, no override): lesson 101 **PASS-ON-TARGET** — `obj_manipulation` 0.82/zero defects, only the known connected-text blocker remains. Lesson 74 matched baseline. Lesson 100 r3 showed a **NEW** `obj_manipulation` veto (0.28) — surfaced as a blocking finding, not chased with a retry loop per the task's rules. **Owner chose a one-time confirm re-run over an immediate code fix**; r4 came back 0.42/zero defects, baseline restored — adjudicated as single-sample judge noise (documented in `.superpowers/sdd/lesson101-fix-traceability.md`, both runs shown honestly). Follow-up chip queued (`task_15a12b24`) since er/est manipulation quality still sits in a weak 0.28-0.46 band across 3 runs — the mechanism (multi-hop suffix lessons keep the old chain wording despite rendering the same base-anchored add-the-ending form) is understood even though r3 itself was noise.
- Fable 5 final whole-branch review: READY TO MERGE. Traceability audit re-reproduced both defects' RED evidence behaviorally (reverted each fix, confirmed the named tests fail, restored). All 5 exit criteria met.
- `/simplify` pass (4 parallel cleanup agents — reuse/simplification/efficiency/altitude): 2 findings hit independently by multiple agents (chain-step `ActivityItem` construction duplicated between `adapt/engine.py` and `adapt/llm_adapt.py`; `_chain_shape` hand-rolled arrow-splitting instead of reusing the canonical suffix parser). Fixed both — the parser-reuse fix also closed a latent inaccuracy the Fable reviewer had flagged as a minor edge case (the old regex matched bare hyphens as chain arrows). Renamed `_chain_shape` → `_manipulation_chain_shape` (cheap half of a "don't imply general-purpose" altitude finding). Deleted one duplicate test helper. Skipped 2 findings with reasons logged inline (dict-lookup-for-2-values = stylistic wash; extending the judge guard to a second severe-defect gloss = a behavior change needing its own live verification, out of `/simplify`'s scope — folded into the same follow-up chip). Verified zero fact drift on real lesson 74/100/101 data; gates green throughout (`8a26042`).
- Gotcha: none new this session — G17/G19 discipline (full typecheck, no subagent Agent-tool nesting) held across all 4 dispatches with zero violations.

**What's next:** merge `fix/lesson101-uat-d48-d49` to `main` (in progress this session). Queue unchanged otherwise: passage-split follow-up (task_202def01, sole remaining judge blocker on both lessons); extend suffix-aware manipulation wording to multi-hop suffix lessons (task_15a12b24, new this session); 2-of-3 judge sampling + judge visibility of render-layer facts (Q5); `--record-results` ingestion; gpt-5.4 → gpt-5.6-terra swap still paused.
...
### Session 62 — 2026-07-16 (lesson-101 UAT: D48/D49 fixed, first complexity-matched model-selection cycle)

**Participants:** Claude controller + Sonnet 5 (Tasks 1-2) + Opus 4.8 (Tasks 3-4) + Fable 5 (final review) — first cycle with per-task model assignment instead of uniform Sonnet 5; owner (D49 scope, L100 regression adjudication, simplify/merge direction).

**What happened:**
- Lesson-101 UAT (no override) surfaced 2 pre-render defects, not just the known connected-text blocker: `obj_manipulation` failed new. Root-caused via live repro: **D48** — `adapt/llm_adapt.py`'s model-authored-items path never applies the coverage evaluator's anti-gaming chain-evidence stamp, so an LLM-planned word_chain could never satisfy manipulation coverage on ANY suffix/letter-chain lesson. **D49** — the manipulation cell's `sufficiency_rule` describes a multi-hop chain that single, non-comparative suffixes (`-ly`) structurally cannot form; UFLI's corpus gives single-hop pairs only.
- Spec v1 mis-targeted D49 (proposed relaxing the deterministic evaluator, which already passed single-hop packages) — caught by an owner double-check request before planning proceeded; v2 correctly retargeted the judge's handed facts.
- Plan + subagent-driven execution (4 tasks, complexity-matched models): word_chain routed through the deterministic stamped parsers + suffix-parser parity (`45f9529`); planner prompt/retry-feedback teach the arrow-words format (`e2e16b6`); manipulation cell gains chain-shape detection + shape-aware wording + judge guard (`eb2d87e`). All task reviews clean, 0 Critical/Important, RED evidence independently re-reproduced by reviewers.
- Live acceptance: lesson 101 ships clean on manipulation (0.82/0 defects); lesson 74 matched baseline; lesson 100 showed a one-run manipulation veto (r3, 0.28) that did NOT reproduce on an owner-directed confirm run (r4, 0.42/0 defects) — adjudicated as single-sample judge noise, both runs documented honestly. Follow-up chip queued to extend shape-aware wording to multi-hop suffix lessons (er/est quality band still weak, 0.28-0.46 across 3 runs).
- Fable 5 final review: READY TO MERGE, traceability audit behaviorally re-reproduced both defects' RED evidence.
- `/simplify` (4 parallel agents): fixed 2 findings hit by multiple agents independently (chain-step item construction deduped between `adapt/engine.py`/`adapt/llm_adapt.py`; `_chain_shape` now reuses the canonical suffix parser instead of a hand-rolled arrow-split that had also been mismatching bare hyphens as chain arrows) plus 1 duplicate test helper removed; skipped 2 findings with logged reasons (stylistic wash; behavior-changing judge-guard extension deferred to the same follow-up chip). Zero fact drift verified on real lesson data; gates green throughout (`8a26042`).
